...
def approve(record_id):
    ...
    approve = Approver(
        sales_order_customer_id=record_id,
...
def unlock(record_id):
    ...
    approve = Approver.query.filter_by(customer_id=record_id).first()
...
def autocomplete():
    rows = [row for row in Obj.query.order_by(Obj.customer_name).all()]
