...
def home():
    ...
    plastic_label_receipts = PlasticLabelReceipt.query.filter(
        PlasticLabelReceipt.record_date.between(date_from, date_to)).order_by(
        PlasticLabelReceipt.record_date.desc(), PlasticLabelReceipt.id.desc()
        ).all()
        

    context = {
        "plastic_label_receipts": plastic_label_receipts,
        "date_from": date_from,
        "date_to": date_to,
    }
...
def add():
    ...
    if request.method == "POST":
        ...
        if form.validate_on_submit():
            ...
            form.save()
            flash(f"{form.plastic_label_receipt_number} has been added.", category="success")
        ...
        form.plastic_label_receipt_number = next_control_number(
            obj=PlasticLabelReceipt, 
            control_number_field="plastic_label_receipt_number"
            )
...
def edit(record_id):   
    ...
    plastic_label_receipt = PlasticLabelReceipt.query.get_or_404(record_id)
    ...
    if request.method == "POST":
        ...
        if form.validate_on_submit():
            ...
            if cmd_button == "Submit for Printing":
                flash(f"{form.plastic_label_receipt_number} has been submitted for printing.", category="success")
                return redirect(url_for(f'{app_name}.edit', record_id=form.id))
            elif cmd_button == "Save Draft":
                flash(f"{form.plastic_label_receipt_number} has been updated.", category="success")
                return  redirect(url_for(f'{app_name}.home'))

    else:
        form = Form()
        form.populate(plastic_label_receipt)
    ...
    context = {
        ...
        "measure_dropdown": measure_dropdown,
        "vendor_dropdown": vendor_dropdown,
        "plastic_label_receipt": plastic_label_receipt,       
    }
...
def delete(record_id):
    ...
    obj = PlasticLabelReceipt.query.get_or_404(record_id)
    details = PlasticLabelReceiptDetail.query.filter(PlasticLabelReceiptDetail.plastic_label_receipt_id==record_id).all()
    ...
    try:
        ...
        db.session.commit()
        flash(f"{obj.plastic_label_receipt_number} has been deleted.", category="success")
...
def cancel(record_id):   
    plastic_label_receipt = PlasticLabelReceipt.query.get_or_404(record_id)
    plastic_label_receipt.cancelled = str(datetime.datetime.today())[:10]
    db.session.commit()
    flash(f"{plastic_label_receipt.plastic_label_receipt_number} has been cancelled.", category="success")
...
def unlock(record_id):
    if current_user.admin:   
        plastic_label_receipt = PlasticLabelReceipt.query.get_or_404(record_id)
        plastic_label_receipt.submitted = ""
        plastic_label_receipt.cancelled = ""
        db.session.commit()
        flash(f"{plastic_label_receipt.plastic_label_receipt_number} has been unlocked.", category="success")
    else:
