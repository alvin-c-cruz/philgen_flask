...
def home():
    rows = Obj.query.order_by(Obj.lithography_name).all()
...
def approve(record_id):
    ...
    arg = {
        "lithography_id": record_id,
        "user_id": current_user.id
    }

    approve = Approver(**arg)

    db.session.add(approve)
    db.session.commit()

    flash(f"Approved: {obj.lithography_name}", category="success")
    return redirect(url_for(f'{app_name}.home'))   
...
def unlock(record_id):
    ...
    approve = Approver.query.filter_by(lithography_id=record_id).first()
    
    db.session.delete(approve)
    db.session.commit()

    flash(f"Unlocked: {obj.lithography_name}", category="error")
    return redirect(url_for(f'{app_name}.home'))   
