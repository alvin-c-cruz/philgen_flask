...
from .models import PlasticLabelAdjustment, PlasticLabelAdjustmentDetail, UserPlasticLabelAdjustment as Preparer
...
class SubForm:
    id: int = 0
    plastic_label_adjustment_id:int = 0
    quantity: float = 0
    measure_id: int = 0
    ...
    def populate(self, row):
        self.id = row.id
        self.plastic_label_adjustment_id = row.plastic_label_adjustment_id
...
class Form:
    id: int = None
    record_date: str = ""
    plastic_label_adjustment_number: str = ""
    notes: str = ""
    submitted: str = ""
    cancelled: str = ""
    locked: bool = False
    ...
    def save(self):
        if self.id is None:
            # Add a new record
            new_record = PlasticLabelAdjustment(
                record_date=self.record_date,
                plastic_label_adjustment_number=self.plastic_label_adjustment_number,
                notes=self.notes
                )
            ...
            for _, detail in self.details:
                if detail.is_dirty():
                    new_detail = PlasticLabelAdjustmentDetail(
                        plastic_label_adjustment_id=new_record.id,
            ...
            record = PlasticLabelAdjustment.query.get(self.id)
            if record:
                preparer = Preparer.query.filter_by(plastic_label_adjustment_id=self.id).first()
                preparer.user_id = self.user_prepare_id

                record.record_date = self.record_date
                record.plastic_label_adjustment_number = self.plastic_label_adjustment_number
                record.notes = self.notes
                record.submitted = self.submitted
                
                details = PlasticLabelAdjustmentDetail.query.filter(PlasticLabelAdjustmentDetail.plastic_label_adjustment_id==self.id)
                for detail in details:
                    db.session.delete(detail)

                for _, detail in self.details:
                    if detail.is_dirty():
                        row_detail = PlasticLabelAdjustmentDetail(
                            plastic_label_adjustment_id=record.id,
                            quantity=detail.quantity,
                            measure_id=detail.measure_id,
                            plastic_label_id=detail.plastic_label_id,
                            side_note=detail.side_note
    ...
    def populate(self, obj):
        ...
        self.plastic_label_adjustment_number = obj.plastic_label_adjustment_number
        self.notes = obj.notes
        self.submitted = obj.submitted
        self.cancelled = obj.cancelled

        for i, row in enumerate(obj.plastic_label_adjustment_details):
            subform = SubForm()
            subform.populate(row)
            self.details[i] = (i, subform)

    def post(self, request_form):
        self.id = request_form.get('plastic_label_adjustment_id')
        self.record_date = request_form.get('record_date')
        self.plastic_label_adjustment_number = request_form.get('plastic_label_adjustment_number')
    ...
    def validate_on_submit(self):
        ...
        if not self.plastic_label_adjustment_number:
            self.errors["plastic_label_adjustment_number"] = "Please type adjustment number."
        else:
            duplicate = PlasticLabelAdjustment.query.filter(func.lower(PlasticLabelAdjustment.plastic_label_adjustment_number) == func.lower(self.plastic_label_adjustment_number), PlasticLabelAdjustment.id != self.id).first()
            if duplicate:
                self.errors["plastic_label_adjustment_number"] = "Adjustment number is already used, please verify."        
