...
from .models import UserDeliveryReceiptExtra as Preparer
...
class SubForm:
    id: int = 0
    delivery_receipt_extra_id:int = 0
    quantity: float = 0
    measure_id: int = 0
    product_id: int = 0
    so_number: str = ""
...
class Form:
    id: int = None
    record_date: str = ""
    delivery_receipt_extra_number: str = ""
    po_number: str = ""
    customer_id: int = 0
    salesman: str = ""
    prepared_by: str = ""
    ...
    def _save(self):
        if self.id is None:
            ...
            preparer = Preparer(
                delivery_receipt_extra_id=new_record.id,
            ...
            if record:
                preparer = Preparer.query.filter_by(delivery_receipt_extra_id=self.id).first()
    ...
    def _validate_on_submit(self):
        ...
        if not self.delivery_receipt_extra_number:
            self.errors["delivery_receipt_extra_number"] = "Please type delivery receipt number."
        else:
            duplicate = Obj.query.filter(
                func.lower(
                    Obj.delivery_receipt_extra_number
                    ) == func.lower(self.delivery_receipt_extra_number), 
                    Obj.id != self.id
                    ).first()
            if duplicate:
                self.errors["delivery_receipt_extra_number"] = "Delivery receipt number is already used, please verify."        
