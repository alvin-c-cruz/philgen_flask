...
from .models import PurchaseOrder, PurchaseOrderDetail, UserPurchaseOrder as Preparer
...
class SubForm:
    ...
    def populate(self, row):
        ...
        self.purchase_order_id = row.purchase_order_id
...
class Form:
    ...
    def save(self):
        if self.id is None:
            ...
            for _, detail in self.details:
                if detail.is_dirty():
                    new_detail = PurchaseOrderDetail(
                        purchase_order_id=new_record.id,
            ...
            record = PurchaseOrder.query.get(self.id)
            if record:
                preparer = Preparer.query.filter_by(purchase_order_id=self.id).first()
                ...
                details = PurchaseOrderDetail.query.filter(PurchaseOrderDetail.purchase_order_id==self.id)
                for detail in details:
                    db.session.delete(detail)

                for _, detail in self.details:
                    if detail.is_dirty():
                        row_detail = PurchaseOrderDetail(
    ...
    def populate(self, obj):
        ...
        for i, row in enumerate(obj.purchase_order_details):
            subform = SubForm()
    ...
    def validate_on_submit(self):
        ...
        if not self.purchase_order_number:
            ...
            duplicate = PurchaseOrder.query.filter(func.lower(PurchaseOrder.purchase_order_number) == func.lower(self.purchase_order_number), PurchaseOrder.id != self.id).first()
            if duplicate:
