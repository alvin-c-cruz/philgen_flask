...
from .models import PlasticRawMaterialIssuance as Obj, PlasticRawMaterialIssuanceDetail as ObjDetail
...
def add():
    ...
    if request.method == "POST":
        ...
        if form.validate_on_submit():
            ...
            form.save()
            flash(f"{form.plastic_raw_material_issuance_number} has been added.", category="success")
        ...
        form.plastic_raw_material_issuance_number = next_control_number(
            obj=Obj, 
...
def edit(record_id):   
    ...
    plastic_raw_material_issuance = Obj.query.get_or_404(record_id)
    ...
    if request.method == "POST":
        ...
        form = Form()
        form.populate(plastic_raw_material_issuance)

    context = {
        "form": form,
        "plastic_raw_material_dropdown": plastic_raw_material_dropdown,
        "measure_dropdown": measure_dropdown,
        "plastic_raw_material_issuance": plastic_raw_material_issuance,       
    }
...
def delete(record_id):
    ...
    details = ObjDetail.query.filter(ObjDetail.plastic_raw_material_issuance_id==record_id).all()
...
def cancel(record_id):   
    plastic_raw_material_issuance = Obj.query.get_or_404(record_id)
    plastic_raw_material_issuance.cancelled = str(datetime.datetime.today())[:10]
    db.session.commit()
    flash(f"{plastic_raw_material_issuance.plastic_raw_material_issuance_number} has been cancelled.", category="success")
...
def unlock(record_id):
    if current_user.admin:   
        plastic_raw_material_issuance = Obj.query.get_or_404(record_id)
        plastic_raw_material_issuance.submitted = ""
        plastic_raw_material_issuance.cancelled = ""
        db.session.commit()
        flash(f"{plastic_raw_material_issuance.plastic_raw_material_issuance_number} has been unlocked.", category="success")
    else:
