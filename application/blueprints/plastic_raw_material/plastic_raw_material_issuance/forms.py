...
from .models import PlasticRawMaterialIssuance, PlasticRawMaterialIssuanceDetail, UserPlasticRawMaterialIssuance as Preparer
...
class SubForm:
    id: int = 0
    plastic_raw_material_issuance_id:int = 0
    quantity: float = 0
    measure_id: int = 0
    ...
    def populate(self, row):
        self.id = row.id
        self.plastic_raw_material_issuance_id = row.plastic_raw_material_issuance_id
...
class Form:
    id: int = None
    record_date: str = ""
    plastic_raw_material_issuance_number: str = ""
    notes: str = ""
    submitted: str = ""
    cancelled: str = ""
    locked: bool = False
    ...
    def save(self):
        if self.id is None:
            # Add a new record
            new_record = PlasticRawMaterialIssuance(
                record_date=self.record_date,
                plastic_raw_material_issuance_number=self.plastic_raw_material_issuance_number,
                notes=self.notes
                )
            ...
            for _, detail in self.details:
                if detail.is_dirty():
                    new_detail = PlasticRawMaterialIssuanceDetail(
                        plastic_raw_material_issuance_id=new_record.id,
            ...
            record = PlasticRawMaterialIssuance.query.get(self.id)
            if record:
                preparer = Preparer.query.filter_by(plastic_raw_material_issuance_id=self.id).first()
                preparer.user_id = self.user_prepare_id

                record.record_date = self.record_date
                record.plastic_raw_material_issuance_number = self.plastic_raw_material_issuance_number
                record.notes = self.notes
                record.submitted = self.submitted
                
                details = PlasticRawMaterialIssuanceDetail.query.filter(PlasticRawMaterialIssuanceDetail.plastic_raw_material_issuance_id==self.id)
                for detail in details:
                    db.session.delete(detail)

                for _, detail in self.details:
                    if detail.is_dirty():
                        row_detail = PlasticRawMaterialIssuanceDetail(
                            plastic_raw_material_issuance_id=record.id,
                            quantity=detail.quantity,
                            measure_id=detail.measure_id,
                            plastic_raw_material_id=detail.plastic_raw_material_id,
                            side_note=detail.side_note
    ...
    def populate(self, obj):
        ...
        self.plastic_raw_material_issuance_number = obj.plastic_raw_material_issuance_number
        self.notes = obj.notes
        self.submitted = obj.submitted
        self.cancelled = obj.cancelled

        for i, row in enumerate(obj.plastic_raw_material_issuance_details):
            subform = SubForm()
            subform.populate(row)
            self.details[i] = (i, subform)

    def post(self, request_form):
        self.id = request_form.get('plastic_raw_material_issuance_id')
        self.record_date = request_form.get('record_date')
        self.plastic_raw_material_issuance_number = request_form.get('plastic_raw_material_issuance_number')
    ...
    def validate_on_submit(self):
        ...
        if not self.plastic_raw_material_issuance_number:
            self.errors["plastic_raw_material_issuance_number"] = "Please type issuance number."
        else:
            duplicate = PlasticRawMaterialIssuance.query.filter(func.lower(PlasticRawMaterialIssuance.plastic_raw_material_issuance_number) == func.lower(self.plastic_raw_material_issuance_number), PlasticRawMaterialIssuance.id != self.id).first()
            if duplicate:
                self.errors["plastic_raw_material_issuance_number"] = "Issuance number is already used, please verify."        
