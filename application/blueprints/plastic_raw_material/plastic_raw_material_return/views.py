...
from .models import PlasticRawMaterialReturn as Obj, PlasticRawMaterialReturnDetail as ObjDetail
from .forms import Form
from .. plastic_raw_material import PlasticRawMaterial
from ... measure import Measure
from .. plastic_raw_material_status import PlasticRawMaterialStatus
...
def add():
    ...
    plastic_raw_material_status_dropdown = [{"id": record.id, "status_name": record.plastic_raw_material_status_name} for record in PlasticRawMaterialStatus.query.order_by('plastic_raw_material_status_name').all() if record.active]
    ...
    if request.method == "POST":
        ...
        if form.validate_on_submit():
            ...
            form.save()
            flash(f"{form.plastic_raw_material_return_number} has been added.", category="success")
        ...
        form.plastic_raw_material_return_number = next_control_number(
            obj=Obj, 
            control_number_field="plastic_raw_material_return_number"
            )

    context = {
        "form": form,
        "plastic_raw_material_dropdown": plastic_raw_material_dropdown,
        "plastic_raw_material_status_dropdown": plastic_raw_material_status_dropdown,
...
def edit(record_id):   
    ...
    if request.method == "POST":
        ...
        if form.validate_on_submit():
            ...
            if cmd_button == "Submit for Printing":
                flash(f"{form.plastic_raw_material_return_number} has been submitted for printing.", category="success")
                return redirect(url_for(f'{app_name}.edit', record_id=form.id))
            elif cmd_button == "Save Draft":
                flash(f"{form.plastic_raw_material_return_number} has been updated.", category="success")
    ...
    context = {
        ...
        "measure_dropdown": measure_dropdown,
        "plastic_raw_material_status_dropdown": plastic_raw_material_status_dropdown,
        "plastic_raw_material_return": obj,       
    }
...
def delete(record_id):
    ...
    obj = Obj.query.get_or_404(record_id)
    details = ObjDetail.query.filter(ObjDetail.plastic_raw_material_return_id==record_id).all()
    ...
    try:
        ...
        db.session.commit()
        flash(f"{obj.plastic_raw_material_return_number} has been deleted.", category="success")
...
def cancel(record_id):   
    ...
    db.session.commit()
    flash(f"{obj.plastic_raw_material_return_number} has been cancelled.", category="success")
...
def unlock(record_id):
    if current_user.admin:   
        ...
        obj.cancelled = ""
        db.session.commit()
        flash(f"{obj.plastic_raw_material_return_number} has been unlocked.", category="success")
    else:
