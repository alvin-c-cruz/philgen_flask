...
from .models import Issuance as Obj, IssuanceDetail as ObjDetail
...
def add():
    ...
    if request.method == "POST":
        ...
        if form.validate_on_submit():
            ...
            form.save()
            flash(f"{form.issuance_number} has been added.", category="success")
        ...
        form.issuance_number = next_control_number(
            obj=Obj, 
            control_number_field="issuance_number", 
            record_date=today
            )
        
        form.prepared_by="PATRICK"            
        form.checked_by=""
        form.approved_by=""
...
def edit(record_id):   
    ...
    if request.method == "POST":
        ...
        if form.validate_on_submit():
            ...
            if cmd_button == "Submit for Inventory":
                form.submit()
            ...
            if cmd_button == "Submit for Inventory":
                flash(f"{form.issuance_number} has been submitted.", category="success")
                return redirect(url_for(f'{app_name}.edit', record_id=form.id))
            elif cmd_button == "Save Draft":
                flash(f"{form.issuance_number} has been updated.", category="success")
...
def delete(record_id):
    ...
    obj = Obj.query.get_or_404(record_id)
    details = ObjDetail.query.filter(ObjDetail.issuance_id==record_id).all()
    ...
    try:
        ...
        db.session.commit()
        flash(f"{obj.issuance_number} has been deleted.", category="success")
...
def cancel(record_id):   
    ...
    db.session.commit()
    flash(f"{obj.issuance_number} has been cancelled.", category="success")
